...
pre_damage.py - Copy
=================
Solver for pre damage HJBs, tech III, tech II, tech I
"""
...
DataDir = "./res_data/10damage/xi_a_" + str(xi_a) + "_xi_g_" + str(xi_g) +  "/"
...
for i in range(len(gamma_3_list)):
    ...
    model_i = pickle.load(open(DataDir + "model_tech3_post_damage_gamma_{:.4f}".format(gamma_3_i), "rb"))
...
for i in range(len(gamma_3_list)):
    gamma_3_i = gamma_3_list[i]
    model_i = pickle.load(open(DataDir + "model_tech2_post_damage_gamma_{:.4f}".format(gamma_3_i), "rb"))
...
############################### PRE DAMAGE###################################################
PreDamageDataDir = "./res_data/xi_a_" + str(xi_a) + "_xi_g_" + str(xi_g) +  "/pre_damage_xi_p_{}/".format(xi_p)
if not os.path.exists(PreDamageDataDir):
    os.mkdir(PreDamageDataDir)
print("-------------------------------------------")
...
with open(PreDamageDataDir + "model_tech3_pre_damage", "wb") as f:
    pickle.dump(model_tech3_pre_damage, f)

model_tech3_pre_damage = pickle.load(open(PreDamageDataDir + "model_tech3_pre_damage", "rb"))
...
model_tech1_pre_damage = hjb_pre_tech(
        state_grid=(K, Y_short, L), 
        model_args=model_args, V_post_damage=v_i, 
        tol=1e-8, epsilon=0.01, fraction=0.1, max_iter=20000,
        v0 = np.mean(v_i, axis=0),
        smart_guess=None,
        )
